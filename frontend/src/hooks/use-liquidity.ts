 ...
 */
const STARKNET_PRIME = BigInt("3618502788666131106986593281521497120414687020801267626233049500247285301248");

const i32ToFelt252 = (value: number): string => {
  if (value < 0) {
    // Negative: PRIME - abs(value)
    return (STARKNET_PRIME - BigInt(Math.abs(value))).toString();
  }
...
export function useLiquidity() {
  ...
  const { notes, removeNote, addNote, addTransaction, updateTransaction } = usePortfolioStore()
  ...
  const mintLiquidity = useCallback(async (
    ...
    try {
      ...
      console.log("[Frontend] 🔍 Verifying tree state and searching for valid note...")
      ...
      if (treeInfo.leaf_count === 0) {
        throw new Error("No deposits found in the Merkle tree. Please deposit tokens first.")
      }
      
      console.log("[Frontend] 📋 Available notes in portfolio:", notes.length)
      ...
      const candidateNotes = notes.filter(note => {
        ...
        if (note.index < 0 || note.index >= treeInfo.leaf_count) {
          console.log(`[Frontend] ⚠️ Skipping note with invalid index: ${note.index} (valid: 0-${treeInfo.leaf_count - 1})`)
          return false
        }
        
        // Must have sufficient balance
        if (note.amount < amount) {
          console.log(`[Frontend] ⚠️ Skipping note with insufficient balance: ${note.amount.toString()} < ${amount.toString()}`)
          return false
        }
        
        // Must match token if specified
        if (tokenAddress && note.tokenAddress !== tokenAddress) {
          console.log(`[Frontend] ⚠️ Skipping note with different token: ${note.tokenAddress} !== ${tokenAddress}`)
          return false
        }
      ...
      if (candidateNotes.length === 0) {
        const errorMsg = tokenAddress
          ? `No valid notes found with sufficient balance (${amount.toString()}) for token ${tokenAddress}. ` +
            `Tree has ${treeInfo.leaf_count} leaves (indices 0-${treeInfo.leaf_count - 1}). ` +
      ...
      console.log("[Frontend] 🔍 Validating note commitments against Merkle tree...")
      ...
      for (const candidate of candidateNotes) {
        ...
        try {
          ...
          const proof = await aspClientInstance.getMerkleProof(candidate.index)
          ...
          const calculatedCommitment = await generateCommitment(candidate.secret, candidate.nullifier, candidate.amount)
        ...
        amount: inputNote.amount.toString(),
              ...
              treeCommitment: treeCommitment.toString().substring(0, 20) + "...",
              matches: "✅ Commitment matches tree",
            })
            break
          } else {
            console.log(`[Frontend] ⚠️ Skipping note at index ${candidate.index}: commitment mismatch`)
            console.log(`  Tree: ${treeCommitment.toString().substring(0, 20)}...`)
            console.log(`  Calculated: ${calculatedCommitment.toString().substring(0, 20)}...`)
          }
        } catch (error: any) {
          console.log(`[Frontend] ⚠️ Skipping note at index ${candidate.index}: validation error - ${error.message}`)
      ...
      console.log("[Frontend] ✅ Selected note:", {
        ...
        commitment: inputNote.commitment.toString().substring(0, 20) + "...",
        tokenAddress: inputNote.tokenAddress,
        validIndex: `✅ Index ${noteIndex} is valid (0-${treeInfo.leaf_count - 1})`,
      ...
      console.log("[Frontend] 📋 Generated position commitment:", positionCommitment.toString())

      // Step 1: Use Merkle Proof (already fetched during validation, or fetch if not available)
      if (!merkleProof) {
        console.log("[Frontend] 🔄 Fetching Merkle proof (not cached during validation)...")
      ...
      if (!validation.isValid) {
        ...
        if (validation.isLegacy) {
          ...
          const treeCommitmentStr = validation.treeCommitment?.toString() || "unknown"
          const calculatedStr = validation.calculatedCommitment?.toString() || "unknown"
          const legacyStr = validation.legacyCommitment?.toString() || "unknown"
      ...
      const changeAmount = inputNote.amount >= liquidity 
        ? inputNote.amount - liquidity 
        : 0n
      
      if (changeAmount < 0n) {
        throw new Error(`Insufficient balance: need ${liquidity.toString()}, have ${inputNote.amount.toString()}`)
      }
      ...
      const pathIndices = Array.isArray(merkleProof.path_indices)
        ? merkleProof.path_indices.map((p: any) => {
            const num = typeof p === 'number' ? p : parseInt(p.toString(), 10)
            if (num !== 0 && num !== 1) {
              throw new Error(`Invalid pathIndices value: ${num}. Must be 0 or 1.`)
            }
            return num
          })
        : []
      ...
      if (pathElements.length === 0 || pathIndices.length === 0) {
        throw new Error('Invalid Merkle proof: pathElements or pathIndices is empty')
      }
      ...
      if (Array.isArray(proof) && proof.length > 8) {
        ...
        console.warn(`[Frontend] ⚠️  Proof had ${proof.length} elements, extracted first 8`)
      }

      if (!Array.isArray(extractedProof) || extractedProof.length !== 8) {
        throw new Error(`Invalid proof length: expected 8 elements, got ${extractedProof?.length || 0}`)
      }

      if (!Array.isArray(publicInputs)) {
        throw new Error(`Invalid public inputs: expected array, got ${typeof publicInputs}`)
      }
      ...
      if (publicInputs.length !== 7) {
        throw new Error(`Invalid public inputs length: expected 7 elements (LP circuit), got ${publicInputs.length}. Public inputs: [${publicInputs.join(", ")}]`)
      }
      
      console.log(`[Frontend] 📋 Raw public inputs received from backend:`, publicInputs)
      ...
      const bn254ToFelt252 = (value: string | bigint): string => {
        try {
          ...
          if (bigValue > BN254_MODULUS / 2n) {
            ...
            console.warn(`[Frontend] ⚠️  Converted BN254 negative to felt252: ${bigValue.toString()} -> ${felt252Value.toString()}`);
          ...
          console.warn(`[Frontend] ⚠️  Could not convert BN254 to felt252 for value: ${value}`, e);
      ...
      const applyFeltModulo = (value: string): string => {
        try {
          ...
          if (bigValue >= STARKNET_FELT_MAX) {
            ...
            console.warn(`[Frontend] ⚠️  Applied felt252 modulo to value: ${value} -> ${moduloValue.toString()}`);
            return moduloValue.toString();
          }
          return value;
        } catch (e) {
          console.warn(`[Frontend] ⚠️  Could not parse value as BigInt: ${value}`);
          return value;
        }
      };

      // Convert all public inputs from BN254 to felt252
      console.log(`[Frontend] 🔄 Converting public inputs from BN254 to felt252...`);
      const convertedPublicInputs = publicInputs.map((val: any, idx: number) => {
        const original = val.toString();
        const converted = bn254ToFelt252(val);
        if (converted !== original) {
          console.log(`[Frontend] 🔄 Public input[${idx}] converted: ${original} -> ${converted}`);
        }
        return converted;
      });
      console.log(`[Frontend] ✅ Converted public inputs (felt252):`, convertedPublicInputs);
      ...
      const normalizedPublicInputs = convertedPublicInputs.map(applyFeltModulo);
      ...
      if (hasOverflow) {
        throw new Error(`[Frontend] ❌ OVERFLOW DETECTED! Some values still exceed felt252 max after normalization.`);
      } else {
      ...
      console.log(`[Frontend] 📋 Executing mint liquidity with manual calldata`)
      console.log(`[Frontend] 📋 Contract: ${CONFIG.ZYLITH_CONTRACT}`)
      console.log(`[Frontend] 📋 Proof: ${normalizedProof.length} elements`)
      console.log(`[Frontend] 📋 Public inputs: ${normalizedPublicInputs.length} elements`)
      ...
      try {
        ...
        console.log("[Frontend] ✅ Contract is accessible, merkle root:", merkleRoot.toString())
        ...
        if (!contract.private_mint_liquidity) {
          ...
          const method = abiArray.find((item: any) => 
            item.name === "private_mint_liquidity" && item.type === "function"
          )
          
          if (!method) {
            // Log all function names for debugging
            const allFunctions = abiArray.filter((item: any) => item.type === "function")
            console.error("[Frontend] ❌ Available functions:", allFunctions.map((f: any) => f.name).join(", "))
        ...
        console.warn("[Frontend] ⚠️ Continuing despite verification warning - will attempt transaction")
      }
      ...
      if (typeof tickLower !== 'number' || typeof tickUpper !== 'number') {
        throw new Error(`Invalid tick values: tickLower=${tickLower}, tickUpper=${tickUpper}`)
      }
      ...
      if (normalizedPublicInputs.length !== 7) {
        throw new Error(`Invalid public_inputs length: expected 7, got ${normalizedPublicInputs.length}`)
      }
      ...
      normalizedPublicInputs.forEach((val, idx) => {
        try {
          ...
          if (bigVal < 0n || bigVal >= STARKNET_FELT_MAX) {
            throw new Error(`public_inputs[${idx}] is out of felt252 range: ${val}`)
          }
        } catch (e) {
          if (e instanceof Error && e.message.includes('out of felt252 range')) {
            throw e
          }
          throw new Error(`public_inputs[${idx}] could not be parsed as BigInt: ${val}`)
        }
      })
      ...
      const FELT_MAX = BigInt("3618502788666131106986593281521497120414687020801267626233049500247285301248");
      ...
      const felt252ToI32 = (value: string): number => {
        ...
        if (bigValue > FELT_MAX / 2n) {
          ...
          if (negativeValue < I32_MIN_VAL || negativeValue > I32_MAX_VAL) {
            throw new Error(`Value ${negativeValue} is out of i32 range (${I32_MIN_VAL} to ${I32_MAX_VAL})`);
          }
          ...
          if (bigValue > I32_MAX_VAL) {
            throw new Error(`Value ${bigValue} is out of i32 range (${I32_MIN_VAL} to ${I32_MAX_VAL})`);
          }
      ...
      console.log(`[Frontend] 📋 Transaction parameters (i32 → felt252 conversion):`)
      console.log(`  - tick_lower: ${tickLower} (i32) → ${tickLowerFelt} (felt252)`)
      console.log(`  - tick_upper: ${tickUpper} (i32) → ${tickUpperFelt} (felt252)`)
      ...
      if (tickLowerFelt !== normalizedPublicInputs[2] || tickUpperFelt !== normalizedPublicInputs[3]) {
        console.warn(`[Frontend] ⚠️  Warning: Converted felt252 values don't match public_inputs!`)
        console.warn(`  - tick_lower: converted=${tickLowerFelt}, public_inputs[2]=${normalizedPublicInputs[2]}`)
        console.warn(`  - tick_upper: converted=${tickUpperFelt}, public_inputs[3]=${normalizedPublicInputs[3]}`)
      }
      ...
      console.log(`[Frontend] 📋 Executing mint liquidity with Starknet.js Contract instance...`)
      console.log(`[Frontend] 📋 Parameters:`)
      console.log(`  - proof: Array<felt252> (${normalizedProof.length} elements)`)
      console.log(`  - public_inputs: Array<felt252> (${normalizedPublicInputs.length} elements)`)
  ...
  }, [account, provider, aspClientInstance, removeNote, addNote, addTransaction, updateTransaction, addPosition, updatePosition, getPosition])
